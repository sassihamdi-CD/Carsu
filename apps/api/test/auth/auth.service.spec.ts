...
describe('AuthService', () => {
  ...
  it('signup maps unique email to ConflictException', async () => {
    ...
    // Assert
    await expect(service.signup('a@b.c', 'Strongpass1')).rejects.toBeInstanceOf(
      ConflictException,
    );
  });
  ...
  it('login throws Unauthorized for unknown user', async () => {
    ...
    (prisma.user.findUnique as jest.Mock).mockResolvedValue(null);
    // Assert
    await expect(service.login('x@y.z', 'pass')).rejects.toBeInstanceOf(
  ...
  it('getProfile returns user and memberships', async () => {
    ...
    (prisma.user as any).findUnique = jest
      .fn()
      .mockResolvedValue({ id: 'u1', email: 'a@b.c' });
    (prisma.userTenant as any).findMany = jest
      .fn()
      .mockResolvedValue([
