...
export class RealtimeService {
   ...
   */
  emitToBoard(tenantId: string, boardId: string, event: string, payload: unknown) {
    ...
    if (!this.gateway.server || !this.gateway.server.sockets) {
      this.logger.warn(`Server not ready, skipping ${event} to board ${boardId}`);
      return;
    }
    ...
    this.logger.debug(`Emitting ${event} to room ${room} (${clientCount} client(s))`);
    ...
    if (clientCount === 0) {
      this.logger.warn(`No clients in room ${room}! Event ${event} will not be delivered.`);
    }
  ...
  emitToTenant(tenantId: string, event: string, payload: unknown) {
    ...
    if (!this.gateway.server || !this.gateway.server.sockets) {
      this.logger.warn(`Server not ready, skipping ${event} to tenant ${tenantId}`);
      return;
    }
    ...
    this.logger.debug(`Emitting ${event} to tenant room ${room} (${clientCount} client(s))`);
    
    // Warning if no clients in tenant room (helps identify connection issues)
    if (clientCount === 0) {
      this.logger.warn(`No clients in tenant room ${room}! Event ${event} will not be delivered.`);
    }
