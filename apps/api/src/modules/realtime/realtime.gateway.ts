...
export class RealtimeGateway implements OnGatewayConnection, OnGatewayInit {
  ...
  afterInit(server: Server) {
    ...
    this.logger.log(`Socket.IO server ready on port ${process.env.PORT || 3000}`);
  ...
  async handleConnection(client: Socket) {
    ...
    this.logger.debug(`New connection attempt from ${client.handshake.address}`);
    this.logger.debug(`Handshake auth: ${JSON.stringify(client.handshake.auth)}`);
    this.logger.debug(`Handshake headers (x-tenant-id): ${client.handshake.headers['x-tenant-id']}`);
    ...
    try {
      ...
      const token = (client.handshake.auth?.token || client.handshake.headers['authorization']) as string | undefined;
      ...
      const tenantId = (client.handshake.auth?.tenantId || client.handshake.headers['x-tenant-id']) as string | undefined;
      
      // Debug: Log authentication check results (helpful for debugging connection issues)
      this.logger.debug(`Auth check - token present: ${!!token}, tenantId present: ${!!tenantId}`);
      
      // Security: Require both token and tenantId - missing either is unauthorized
      if (!token || !tenantId) {
        this.logger.warn(`Missing token or tenantId. Token: ${!!token}, TenantId: ${!!tenantId}`);
      ...
      client.on('join_board', (data: { boardId: string }) => {
        ...
        this.logger.log(`User joined board room: ${room}. Total rooms: ${allRooms.join(', ')}`);
      ...
      client.on('disconnect', (reason) => {
        ...
        this.logger.log(`Socket disconnected - user=${payload.sub} tenant=${tenantId} reason=${reason}`);
      });
    } catch (e) {
      ...
      this.logger.warn(`Unauthorized connection from ${client.handshake.address}`);
